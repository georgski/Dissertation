/*
A class representing fuzzy set with plateau on the left hand side
*/
class LeftShoulderSet extends FuzzySet{
	
	public function LeftShoulderSet(leftOffset:float, rightOffset:float, peakPoint:float){
		super(leftOffset, rightOffset, peakPoint);
		representativePoint = leftOffset/2;
	}
	
	//Calculating degree of membership
	public function CalculateDOM(value : float):float{
		if (value == peakPoint) return 1;
		//find DOM if left of center 
		if ( (value >= peakPoint) && (value < (peakPoint + rightOffset)) ) { 
			var grad:float = 1.0 / -rightOffset;
			return grad * (value - peakPoint) + 1;
		}
		//find DOM if right of center 
		else if (value < peakPoint) { 
			return 1; 
		}
		//out of range
		else { 
			return 0; 
		}
	}
}